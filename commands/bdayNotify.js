...
module.exports = {
    ...
    async execute(client, Discord) {
        ...
        var job = new CronJob('0 6 * * *', async() => {

            var mm = new Date().getMonth() + 1;
            var dd = new Date().getDate();            
        
            await api.getBdayUser(dd, mm).then(async (users) => {
                if (users != 'No data found') {
                    ...
                    await client.channels.cache.get(process.env.TARGET_CHANNEL).send('Well, it seems that no one here is celebrating their birthday today!');
                }
